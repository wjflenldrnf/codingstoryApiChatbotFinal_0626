...
const koficApiUrl = `/api/WeekDayMovieList?key=${koficApiKey}&targetDt=${targetDate}&weekGb="0"`;
//const koficApiUrl = `http://kobis.or.kr/kobisopenapi/webservice/rest/boxoffice/searchWeeklyBoxOfficeList.json?key=a00c456adf2ded3c39df93f44cf40503&targetDt=20120101`;
    ...
    $.ajax({
        ...
        success: function(result) {
            try {
                ...
                if (movieData && movieData.boxOfficeResult && movieData.boxOfficeResult.weeklyBoxOfficeList) {
                    let weeklyBoxOfficeList = movieData.boxOfficeResult.weeklyBoxOfficeList;
                    let weeklyShowRange = movieData.boxOfficeResult.showRange;
                    console.log("weeklyShowRange: ", weeklyShowRange);
                    console.log("Weekly Box Office List: ", weeklyBoxOfficeList);

                    if (Array.isArray(weeklyBoxOfficeList)) {
                        weeklyBoxOfficeList.forEach(movie => {
                            ...
                            fetchMovieDetails2(movieTitle, releaseDate, movieCode, audiAcc, rank,weeklyShowRange);
                        });
                    } else {
                        console.error('Weekly Box Office List is not an array: ', weeklyBoxOfficeList);
                    }
                } else {
                    console.error('Missing weeklyBoxOfficeList in response:', movieData);
                }
            } catch (error) {
...
function fetchMovieDetails2(movieTitle, releaseDate, movieCode, audiAcc, rank,weeklyShowRange) {
    // TMDB API 검색 엔드포인트
        ...
        success: function(data) {
            ...
            if (movieDetails) {
                ...
                if (movieOriginalLanguage !== 'ko') {
                    fetchKoreanOverview2(movieDetails.id, movieTitle, posterPath, movieOverview, releaseDate, audiAcc, movieCode, rank,weeklyShowRange);
                } else {
                    // 영화 정보 화면에 표시
                    displayMovieInfo2(movieTitle, posterPath, movieOverview, releaseDate, audiAcc, movieCode, rank,weeklyShowRange);
                }
            }
        },
        error: function(error) {
...
// 영화의 한글 줄거리를 가져오는 함수
function fetchKoreanOverview2(movieId, movieTitle, posterPath, movieOverview, releaseDate, audiAcc, movieCode, rank,weeklyShowRange) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (koreanOverview) {
                ...
                displayMovieInfo2(movieTitle, posterPath, koreanOverview.data.overview, releaseDate, audiAcc, movieCode, rank,weeklyShowRange);
            } else {
...
// 영화 정보를 화면에 표시하는 함수
function displayMovieInfo2(movieTitle, posterPath, movieOverview, releaseDate, audiAcc, movieCode, rank,weeklyShowRange) {
    const posterUrl = `https://image.tmdb.org/t/p/w500/${posterPath}`;
    const showRangeData =`
   <div class="title">
            <h2>주중 박스오피스</h2>
              <p  class="showRange">단위 기간: ${weeklyShowRange}<p>
        </div>`;
    ...
    $('.movieTitle').off('click').on('click', function() {
        ...
        fetchMovieInfo2(movieCodeClicked,clickedMovieTitle,clickedMovieOverview);
    console.log("movieOverview:>>>>>!!!!!!!!!!!!!!!!!!!!!. " + clickedMovieOverview);
...
function fetchMovieInfo2(movieCode,movieTitle,movieOverview) {
